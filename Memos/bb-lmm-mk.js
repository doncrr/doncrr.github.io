...
async function getFirstList(apiV1){
  try {
    ...
    let bbUrl = "http://memoss.hppro1.hpnu.cn/api/v1/memos?creatorId=1"+"&filter=creator_id == 1&pageSize="+AppState.limit;
...
async function getNextList(apiV1){
  try {
    ...
    let bbUrl = "http://memoss.hppro1.hpnu.cn/api/v1/memos?creatorId=1"+"&pageSize="+AppState.limit+"&pageToken="+AppState.offset;
http://memoss.hppro1.hpnu.cn/api/v1/memos?creatorId=1&limit=30
    // 存在标签过滤
...
function getTypeOfMemos(e){
  ...
  let bbUrl = "http://memoss.hppro1.hpnu.cn/api/v1/memos?creatorId=1"+"&filter=tag in [\""+tagName+"\"]&pageSize="+AppState.limit;
  // 标签模式，同时重置页面序列
